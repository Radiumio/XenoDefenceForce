class CfgPatches 
{
	class xdf
	{
		requiredAddons[] = 
		{
			"SSU_Core",
			"TCGM_Girls",
			"A3_Armor_F_Decade",
			"A3_Weapons_F_Decade",
			"A3_Weapons_F",
			"A3_Weapons_F_Exp",
			"A3_Weapons_F_Mod",
			"A3_Weapons_F_Mark",
			"A3_Weapons_F_Tank",
			"A3_Data_F_Tank",
			"A3_Sounds_F",
			"A3_Data_F_Decade",
			"A3_Armor_F_Gamma",
			"A3_Data_F_Enoch_Loadorder"
		};
		version = "1.0";
		author="Radium";
		authorUrl = "https://steamcommunity.com/id/radiumio/";
		units[] = 
		{
			"XDF_Gemini",
			"XDF_Accipiter",
			"XDF_Vulture",
			"XDF_Tusk",
			"XDF_Caiman",
			"XDF_Comanche",
			"XDF_Comanche_Pylons",
			"XDF_Medusa",
			"XDF_Medusa_Ammo",
			"XDF_Medusa_Bench",
			"XDF_Medusa_Cargo",
			"XDF_Medusa_Fuel",
			"XDF_Medusa_Medical",
			"XDF_Medusa_Repair",
			"XDF_Medusa_Transport",
			"XDF_Medusa_Ammo_Pod",
			"XDF_Medusa_Bench_Pod",
			"XDF_Medusa_Cargo_Pod",
			"XDF_Medusa_Transport_Pod",
			"XDF_Medusa_Fuel_Pod",
			"XDF_Medusa_Medical_Pod",
			"XDF_Medusa_Repair_Pod",
			"XDF_Skyhawk",
			"XDF_Dragonfly",
			"XDF_Dragonfly_Pylons",
			"XDF_Dragonfly_Assault",
			"XDF_Dragonfly_DAP",
			"XDF_Merlin",
			"XDF_Mako",
			"XDF_Mako_Unarmed",
			"XDF_Lynx",
			"XDF_Lynx_Unarmed",
			"XDF_Hornet",
			"XDF_Hornet_Armed",
			"XDF_Wraith_IT",
			"XDF_Wraith_VT",
			"XDF_Albatross_IT",
			"XDF_Albatross_VT",
			"XDF_Retributor",
			"XDF_Mars",
			"XDF_Mosquito",
			"XDF_Sentinel",
			"XDF_Guardian",
			"XDF_Reaper",
			"XDF_Kestrel",
			"XDF_Quadbike",
			"XDF_Boar",
			"XDF_Boar_GMG",
			"XDF_Boar_HMG",
			"XDF_Punisher",
			"XDF_Punisher_GMG",
			"XDF_Punisher_HMG",
			"XDF_Jackal",
			"XDF_Jackal_HMG",
			"XDF_Jackal_AT",
			"XDF_Stingray",
			"XDF_Stingray_AT",
			"XDF_Stingray_Minigun",
			"XDF_Puma",
			"XDF_Puma_GMG",
			"XDF_Puma_HMG",
			"XDF_Odin",
			"XDF_Hurricane",
			"XDF_Scoria",
			"XDF_Champion",
			"XDF_Argus",
			"XDF_Commando",
			"XDF_Scorpion",
			"XDF_Huntress",
			"XDF_Dementer",
			"XDF_Kodiak",
			"XDF_Basilisk",
			"XDF_Basilisk_LIC",
			"XDF_Minotaur",
			"XDF_Minotaur_CTWS",
			"XDF_Onyx",
			"XDF_Cyclone_Ammo",
			"XDF_Cyclone_Covered",
			"XDF_Cyclone_Fuel",
			"XDF_Cyclone_Medical",
			"XDF_Cyclone_Repair",
			"XDF_Cyclone",
			"XDF_HEMTT_Flatbed",
			"XDF_HEMTT_Cargo",
			"XDF_HEMTT_Medical",
			"XDF_HEMTT_Fuel",
			"XDF_HEMTT_Repair",
			"XDF_HEMTT_Ammo",
			"XDF_HEMTT_Box",
			"XDF_HEMTT_Transport_Covered",
			"XDF_HEMTT_Transport",
			"XDF_HEMTT_Mover",
			"XDF_Offroad_Comms",
			"XDF_Offroad_Covered",
			"XDF_Van_Ambulance",
			"XDF_Van_Cargo",
			"XDF_Van_Service",
			"XDF_Van_Transport",
			"XDF_Titan",
			"XDF_Ocelot_ATGM",
			"XDF_Ocelot_Radar",
			"XDF_Ocelot_AA",
			"XDF_Ocelot_Cannon",
			"XDF_Trident",
			"XDF_RHIB",
			"XDF_SDV",
			"XDF_Stratoseeker",
			"XDF_Patriot",
			"XDF_Praetorian",
			"XDF_Typhon_VLS",
			"XDF_StaticLance",
			"XDF_StaticSpear",
			"XDF_XM307",
			"XDF_XM307_Raised",
			"XDF_XM307A",
			"XDF_XM312A",
			"XDF_XM312",
			"XDF_XM312_Raised",
			"XDF_M2HMG",
			"XDF_M2HMG_Raised",
			"B_XDF_Uniform_Black",
			"B_XDF_Uniform_Rolled_Black",
			"B_XDF_Uniform_Sleeved_Black",
			"B_XDF_Coveralls_Black",
			"B_XDF_Uniform_Ghillie_Black",
			"B_XDF_Uniform_Tee_Black",
			"U_XDF_Female_Uniform_Black",
			"U_XDF_Female_Uniform_Black_Rolled",
			"U_XDF_Female_Uniform_Black_Bra",
			"B_XDF_Viper_Harness_Black",
			"B_XDF_Viper_Light_Harness_Black",
			"B_XDF_Assault_Pack_Black",
			"B_XDF_LegStrapBag_Black",
			"B_XDF_TacticalPack_Black",
			"B_XDF_Kitbag_Black",
			"B_XDF_RadioBag_Black",
			"B_XDF_Operative",
			"B_XDF_Operative_F",
			"B_XDF_Light_Operative",
			"B_XDF_Light_Operative_F",
			"B_XDF_Combat_Diver",
			"B_XDF_Combat_Diver_F",
			"B_XDF_Autorifleman",
			"B_XDF_Autorifleman_F",
			"B_XDF_Marksman",
			"B_XDF_Marksman_F",
			"B_XDF_Sniper",
			"B_XDF_Sniper_F",
			"B_XDF_Medic",
			"B_XDF_Medic_F",
			"B_XDF_Engineer",
			"B_XDF_Engineer_F",
			"B_XDF_JTAC",
			"B_XDF_JTAC_F",
			"B_XDF_UAV",
			"B_XDF_UAV_F",
			"B_XDF_Grenadier",
			"B_XDF_Grenadier_F",
			"B_XDF_HAT",
			"B_XDF_HAT_F",
			"B_XDF_MS_AT",
			"B_XDF_MS_AT_F",
			"B_XDF_MS_AA",
			"B_XDF_MS_AA_F",
			"B_XDF_Crew",
			"B_XDF_Pilot",
			"B_XDF_Heli_Crew",
			"B_XDF_Jet_Pilot",
			"B_XDF_AX_Operative",
			"B_XDF_AX_Operative_F",
			"B_XDF_AX_Medic",
			"B_XDF_AX_Medic_F",
			"B_XDF_AX_JTAC",
			"B_XDF_AX_JTAC_F",
			"B_XDF_AX_UAV",
			"B_XDF_AX_UAV_F",
			"B_XDF_AX_Gunner",
			"B_XDF_AX_Gunner_F",
			"B_XDF_AX_Hunter",
			"B_XDF_AX_Hunter_F",
			"B_XDF_AX_HAT",
			"B_XDF_AX_HAT_F",
			"B_XDF_AX_Elite_Operative",
			"B_XDF_AX_Elite_Operative_F",
			"B_XDF_AX_Elite_Medic",
			"B_XDF_AX_Elite_Medic_F",
			"B_XDF_AX_Elite_JTAC",
			"B_XDF_AX_Elite_JTAC_F",
			"B_XDF_AX_Elite_UAV",
			"B_XDF_AX_Elite_UAV_F",
			"B_XDF_AX_Elite_Gunner",
			"B_XDF_AX_Elite_Gunner_F",
			"B_XDF_AX_Elite_Hunter",
			"B_XDF_AX_Elite_Hunter_F",
			"B_XDF_AX_Elite_HAT",
			"B_XDF_AX_Elite_HAT_F",
			"B_XDF_Xenohunter",
			"B_XDF_Xenohunter_Sharpshooter",
			"B_XDF_Xenohunter_Exterminator",
			"B_XDF_Xenohunter_Corpsman",
			"B_XDF_Officer",
			"B_XDF_Researcher",
			"B_XDF_Research_Operative",
			"B_XDF_Research_Operative_CBRN",
			"B_XDF_Unarmed_M",
			"Flag_TaskForceLotus",
			"Flag_TaskForceArtificer",
			"Flag_TaskForceCeto",
			"Flag_TaskForceDamocles",
			"Flag_TaskForceIon",
			"Flag_XenoDefenceForce",
			"Flag_XenoDefenceForce_Plain",
			"Flag_XNV_Xenohunter",
			"Flag_XNV_Xenohunter_Solid",
			"Flag_PhoenixDivision",
			"Flag_SapphoDivision",
			"Flag_PrydonianDivision",
			"Flag_ArbalestDivision",
			"Flag_LazarusDivision",
			"Flag_AresDivision",
			"Flag_KomodoDivision",
			"Banner_PhoenixDivision",
			"Banner_SapphoDivision",
			"Banner_PrydonianDivision",
			"Banner_ArbalestDivision",
			"Banner_LazarusDivision",
			"Banner_AresDivision",
			"Banner_KomodoDivision",
			"Banner_TaskForceLotus",
			"Banner_TaskForceArtificer",
			"Banner_TaskForceCeto",
			"Banner_TaskForceDamocles",
			"Banner_TaskForceIon",
			"Banner_XenoDefenceForce",
			"Banner_XenoDefenceForce_Plain",
			"Banner_XNV_Xenohunter",
			"Banner_XNV_Xenohunter_Solid",
			"Land_XDF_IRMaskingCover_01",
			"Land_XDF_IRMaskingCover_02",
			"Land_DeconTent_01_XDF",
			"Land_MedicalTent_01_XDF_Closed",
			"Land_MedicalTent_01_XDF_generic_closed",
			"Land_MedicalTent_01_XDF_generic_inner",
			"Land_MedicalTent_01_XDF_generic_open",
			"Land_MedicalTent_01_XDF_generic_outer",
			"Land_ConnectorTent_01_XDF_closed",
			"Land_ConnectorTent_01_XDF_cross",
			"Land_ConnectorTent_01_XDF_open",
			"XDF_MiniSign_Danger_Tall", 
			"XDF_MiniSign_Danger", 
			"XDF_MiniSign_Radioactive_Tall", 
			"XDF_MiniSign_Radioactive", 
			"XDF_Sign_Armoury", 
			"XDF_SignS_Armoury", 
			"XDF_Sign_Command", 
			"XDF_SignS_Command", 
			"XDF_Sign_Danger", 
			"XDF_SignS_Danger", 
			"XDF_Sign_Deployment_Area",
			"XDF_SignS_Deployment_Area",
			"XDF_Sign_Service_Area",
			"XDF_SignS_Service_Area",
			"XDF_Sign_Munition_Depot",
			"XDF_SignS_Munition_Depot",
			"XDF_Sign_Elite_Wolf", 
			"XDF_SignS_Elite_Wolf", 
			"XDF_Sign_Entrance", 
			"XDF_SignS_Entrance", 
			"XDF_Sign_Evac_Zone", 
			"XDF_SignS_Evac_Zone", 
			"XDF_Sign_Exit", 
			"XDF_SignS_Exit", 
			"XDF_Sign_Firing_Range", 
			"XDF_SignS_Firing_Range", 
			"XDF_Sign_Garage", 
			"XDF_SignS_Garage", 
			"XDF_Sign_Live_Fire", 
			"XDF_SignS_Live_Fire", 
			"XDF_Sign_Medical", 
			"XDF_SignS_Medical", 
			"XDF_Sign_PPE_Zone", 
			"XDF_SignS_PPE_Zone", 
			"XDF_Sign_Radioactive", 
			"XDF_SignS_Radioactive", 
			"XDF_Sign_Research", 
			"XDF_SignS_Research", 
			"XDF_Sign_RA_Warning", 
			"XDF_SignS_RA_Warning", 
			"XDF_Sign_TF_Artificer", 
			"XDF_SignS_TF_Artificer", 
			"XDF_Sign_TF_Ceto", 
			"XDF_SignS_TF_Ceto", 
			"XDF_Sign_TF_Damocles", 
			"XDF_SignS_TF_Damocles", 
			"XDF_Sign_TF_Ion", 
			"XDF_SignS_TF_Ion", 
			"XDF_Sign_TF_Lotus",
			"XDF_SignS_PhoenixDivision",  
			"XDF_Sign_PhoenixDivision", 
			"XDF_SignS_SapphoDivision", 
			"XDF_Sign_SapphoDivision", 
			"XDF_SignS_PrydonianDivision", 
			"XDF_Sign_PrydonianDivision", 
			"XDF_SignS_ArbalestDivision", 
			"XDF_Sign_ArbalestDivision",  
			"XDF_SignS_LazarusDivision", 
			"XDF_Sign_LazarusDivision", 
			"XDF_SignS_AresDivision", 
			"XDF_Sign_AresDivision", 
			"XDF_SignS_KomodoDivision", 
			"XDF_Sign_KomodoDivision",
			"XDF_Sign_Warning", 
			"XDF_SignS_Warning", 
			"XDF_Sign_Wolf", 
			"XDF_SignS_Wolf", 
			"XDF_Sign_XDF", 
			"XDF_SignS_XDF", 
			"XDF_Sign_Xenohunter", 
			"XDF_SignS_Xenohunter",
			"B_CargoNet_XDF_ammo_F",
			"Box_XDF_Ammo_F",
			"Box_XDF_Wps_F",
			"Box_XDF_AmmoOrd_F",
			"Box_XDF_Grenades_F",
			"Box_XDF_WpsLaunch_F",
			"Box_XDF_WpsSpecial_F",
			"Box_XDF_Support_F",
			"Box_XDF_Uniforms_F",
			"Box_XDF_Equip_F",
			"Box_XDF_AmmoVeh_F",
			"XDF_Blahaj",
			"XDF_Blahaj_Orange",
			"XDF_Blahaj_Rainbow"
		};
		weapons[] = 
		{
			"U_XDF_Uniform_Black",
			"U_XDF_Uniform_Rolled_Black",
			"U_XDF_Uniform_Sleeved_Black",
			"U_XDF_Coveralls_Black",
			"U_XDF_Uniform_Ghillie_Black",
			"U_XDF_Uniform_Tee_Black",
			"U_XDF_Female_Uniform_Black",
			"U_XDF_Female_Uniform_Black_Rolled",
			"U_XDF_Female_Uniform_Black_Bra",
			"V_XDF_Tactical_Vest_Black",
			"V_XDF_Carrier_Lite_Black",
			"V_XDF_Carrier_Rig_Black",
			"V_XDF_Modular_Carrier_Lite_Black",
			"V_XDF_Modular_Carrier_GL_Rig_Black",
			"V_XDF_Carrier_GL_Rig_Black",
			"V_XDF_Carrier_Special_Rig_Black",
			"H_XDF_Stealth_Combat_Helmet_Black",
			"H_XDF_Stealth_Combat_Helmet_Black_b",
			"H_XDF_Combat_Helmet_Black",
			"H_XDF_Enhanced_Combat_Helmet_Black",
			"H_XDF_Camo_Combat_Helmet_Black",
			"H_XDF_Light_Combat_Helmet_Black",
			"H_XDF_Advanced_Modular_Helmet_HS_Black",
			"H_XDF_Advanced_Modular_Helmet_Ear_Black",
			"H_XDF_Advanced_Modular_Helmet_Chops_Black",
			"H_XDF_Special_Purpose_Helmet_Black",
			"H_XDF_Boonie_HS_Black",
			"H_XDF_Cap_HS_Black",
			"XDF_launch_MRAWS_black"
		};
	};

};